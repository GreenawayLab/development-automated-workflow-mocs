...
# imine region but can show cage aromatic peaks also
def _get_imine_peaks(
    ...
    peaks = filter(
        lambda peak: peak.in_range(6.0, 9.0),
        peaks,
    )
    peaks = _shift_peaks(peaks, reference_shift)
    peaks = _remove_peaks(peaks, solvent_peaks)
...
def add_aldehyde_comments(percentages: dict) -> dict:
    ...
    for key, value in percentages.items():
        if value <= 1:
            comment = "aldehyde is below 1% and categorised as minor"
        elif 1 < value <= 5:
            comment = "aldehyde is between 1-5% and can still be considered for scale-up"
        else:
            comment = "aldehyde is above 5% and conversion is not satisfactory"
...
def process_nmr_file(title_file: Path) -> None:
    ...
    result_json_file = f'/path/to/nmr/data/{title_id}_result.json'
    ...
    result_nmr.to_csv(f'/path/to/nmr/data/{title_id}.csv', index=False)
